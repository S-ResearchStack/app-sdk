...
class PpgIrFileRepositoryTest {
    private val context = mockk<Context>()
    private val splitInterval = DateUtils.HOUR_IN_MILLIS
    private val ppgIrFileRepository =
        spyk(FileRepository(PpgIr::class, context, splitInterval), recordPrivateCalls = true)
    ...
    fun `delete accMeter File throw FileNotFoundException`() {
        ...
        Assert.assertThrows(FileNotFoundException::class.java) {
            ppgIrFileRepository.deleteFile().getOrThrow()
        }
    }
    ...
    fun `save file throw FileNotFoundException `() {
        ...
        val ppgIr = PpgIr(timestamp = timeStamp)
        Assert.assertThrows(FileNotFoundException::class.java) {
            ppgIrFileRepository.saveAll(listOf(ppgIr))
        }
    }
    ...
    fun `save file throw ClassCastException`() {
        ...
        val ppgIr: Timestamp = Accelerometer(timestamp = timeStamp)
        Assert.assertThrows(ClassCastException::class.java) {
            ppgIrFileRepository.saveAll(listOf(ppgIr as PpgIr))
        }
    }
    ...
    fun `save file throw InvalidObjectException`() {
        ...
        every { ppgIrFileRepository.gson.toJson(0) } throws InvalidObjectException("abc")
    ...
    fun `save file throw ArithmeticException`() {
        ...
        every { ppgIrFileRepository.gson.toJson(25200000) } throws InvalidObjectException("abc")

        Assert.assertThrows(ArithmeticException::class.java) {
            ppgIrFileRepository.saveAll(listOf(PpgIr(timestamp = 1 / 0L)))
        }
    }
    ...
    fun `delete accMeter File after sync file to mobile`() {
        ...
        val ppgIr = PpgIr(timestamp = timeStamp)
        ppgIrFileRepository.saveAll(listOf(ppgIr))
        Assert.assertTrue(ppgIrFileRepository.deleteFile().isSuccess)
    }
    ...
    fun `get completed files should return list with size greater than 0`() {
        ...
        val ppgIr = PpgIr(timestamp = timeStamp)
        ppgIrFileRepository.saveAll(listOf(ppgIr))
        Assert.assertEquals(1, ppgIrFileRepository.getCompletedFiles().size)
    }
}
